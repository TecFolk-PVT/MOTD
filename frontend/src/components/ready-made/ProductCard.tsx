import type { CarouselProduct } from "@/lib/mock/homepage";
...
export function ProductCard({
  ...
  return (
    <div
      className="product-card flex-shrink-0 transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)]"
      style={{
        width: `${width}px`,
        ...
        <div className="aspect-[9/9] relative overflow-hidden">
          ...
          {product.tag ? (
            <div className="absolute top-3 xs:top-4 left-3 xs:left-4">
              <span
                className={`[font-family:var(--font-ui)] text-[8px] xs:text-[9px] sm:text-[10px] md:text-[9px] lg:text-[10px] xl:text-[11px] uppercase tracking-[0.24em] px-1.5 xs:px-2 py-0.5 xs:py-1 text-white ${product.tagColor}`}
              >
                {product.tag}
              </span>
            </div>
