...
export default function FabricStorePage() {
  ...
  const [quickViewProduct, setQuickViewProduct] =
    useState<FabricListItem | null>(null);
  ...
  const matchesColorFilter = (
    fabricColor: string[] | string | undefined,
    selectedColors: string[],
  ) => {
    if (selectedColors.length === 0) return true;
    if (!fabricColor) return false;
    const colorsArray = Array.isArray(fabricColor)
      ? fabricColor
      : [fabricColor];
  ...
  let filteredProducts = fabrics.filter((fabric) => {
    if (
      ...
      !filters.categories.includes(fabric.material)
    ) {
      return false;
    }
    ...
    if (filters.priceRange) {
      ...
      if (
        range &&
        (fabric.pricePerMeter < range.min || fabric.pricePerMeter > range.max)
      ) {
        return false;
      }
    }
      ...
      <div className="py-12 sm:py-16 lg:py-24 border-b border-[#E4E0D8] px-4 sm:px-8 lg:px-12">
        <div className="w-full text-left">
          ...
          <p className="[font-family:var(--font-body)] text-[14px] xs:text-[13px] sm:text-[14px] md:text-[13px] lg:text-[14px] xl:text-[15px] 2xl:text-[16px] leading-normal xs:leading-[1.6] text-[#7A7A72] max-w-2xl">
            Exquisite traditional craftsmanship met with contemporary
            minimalism. Explore our curated selection of high-end fabrics
            tailored in the GCC.
          </p>
        </div>
      </div>
            ...
            <>
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {paginatedProducts.map((product) => {
                  ...
                  const materialLabel = formatMaterialLabel(
                    product.material,
                    locale,
                  );
      ...
      {quickViewProduct && (
        <>
          ...
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-2xl bg-[#FDFAF5] border border-[#E4E0D8] rounded-2xl z-50 p-8 max-h-[90vh] overflow-y-auto">
            <button
              onClick={() => setQuickViewProduct(null)}
              className="absolute top-4 right-4 hover:opacity-50 transition-opacity flex items-center justify-center hover:cursor-pointer"
            >
              <svg
                className="w-5 h-5"
                fill="none"
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-2 mb-6">
                  <p>
                    <span className="text-[10px] tracking-[0.16em] uppercase [font-family:var(--font-body)]">
                      City:
                    </span>{" "}
