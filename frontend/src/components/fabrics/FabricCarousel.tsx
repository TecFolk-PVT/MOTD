...
const FILTER_KEYS: FabricFilter[] = ["all", "wool", "silk", "linen", "cashmere"];
...
export function FabricCarousel() {
  const t = useTranslations("FabricCarousel");
  const [activeFilter, setActiveFilter] = useState<FabricFilter>("all");

  const filteredItems = useMemo(
    () => filterFabrics(fabricsCollection, activeFilter),
    [activeFilter],
  );
  ...
  const selectFilter = (filter: FabricFilter) => {
          ...
          >
            ...
            >
              {infiniteItems.map((itemIndex, slideIndex) => {
                const fabric = filteredItems[itemIndex];
                const isLast = slideIndex === infiniteItems.length - 1;
                return (
                  <FabricCard
                    key={`${slideIndex}-${fabric.title}`}
                    fabric={fabric}
                    width={cardWidth}
                    marginRight={spacing}
