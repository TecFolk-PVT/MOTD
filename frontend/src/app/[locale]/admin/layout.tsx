...
export default function AdminLayout({
    ...
    return (
        <div className="relative min-h-screen bg-black text-white">
            ...
            <div
                className={`fixed inset-0 bg-black/50 z-30 transition-opacity duration-300 lg:hidden ${isSidebarOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
                    }`}
                onClick={() => setIsSidebarOpen(false)}
            />
            <aside
                className={`fixed left-0 top-0 w-72 h-full bg-black border-r border-white/10 flex flex-col p-6 z-40 transition-transform duration-300 ease-in-out lg:hidden ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"
                    }`}
            >
                <button
                    onClick={() => setIsSidebarOpen(false)}
